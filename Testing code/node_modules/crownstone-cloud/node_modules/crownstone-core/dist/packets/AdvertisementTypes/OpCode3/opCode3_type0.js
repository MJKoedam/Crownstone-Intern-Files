...
function parseOpCode3_type0(serviceData, data) {
    if (data.length == 16) {
        ...
        if (serviceData.timeIsSet) {
            serviceData.timestamp = Timestamp_1.reconstructTimestamp(new Date().valueOf() * 0.001, serviceData.partialTimestamp);
        }
        else {
            serviceData.timestamp = serviceData.partialTimestamp; // this is now a counter
        }
        // bitmask states
        let globalBitmaskArray = Util_1.Util.getBitMaskUInt8(data[14]);
