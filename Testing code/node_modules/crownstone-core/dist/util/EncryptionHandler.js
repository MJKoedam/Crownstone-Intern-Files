...
class EncryptionHandler {
    static decryptSessionNonce(rawNonce, key) {
        if (key.length !== 16) {
            throw new CrownstoneError_1.CrownstoneError(enums_1.CrownstoneErrorType.INPUT_ERROR, "Invalid Key");
        }
        if (rawNonce.length !== 16) {
            throw new CrownstoneError_1.CrownstoneError(enums_1.CrownstoneErrorType.INPUT_ERROR, "Invalid Payload for sessionNonce decrypting!");
        }
        ...
        if (0xcafebabe === decrypted.readUInt32LE(0)) {
            ...
            throw new CrownstoneError_1.CrownstoneError(enums_1.CrownstoneErrorType.COULD_NOT_VALIDATE_SESSION_NONCE, "Could not validate Session Nonce", 301);
        }
    }
    ...
    static _getKey(userLevel, settings) {
        if (settings.initializedKeys == false && userLevel != enums_1.UserLevel.setup) {
            throw "BleError.COULD_NOT_ENCRYPT_KEYS_NOT_SET";
        }
        let key = null;
        switch (userLevel) {
            case enums_1.UserLevel.admin:
                key = settings.adminKey;
                break;
            case enums_1.UserLevel.member:
                key = settings.memberKey;
                break;
            case enums_1.UserLevel.basic:
                key = settings.basicKey;
                break;
            case enums_1.UserLevel.setup:
...
class EncryptedPackage {
    constructor(data) {
        ...
        if (this.userLevel > 2 && this.userLevel != enums_1.UserLevel.setup) {
            throw 'BleError.INVALID_KEY_FOR_ENCRYPTION';
        }
